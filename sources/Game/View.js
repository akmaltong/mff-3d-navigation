...
export class View
{
    ...
    setFocusPoint()
    {
        // Default spawn position — center between Лаунж-зона 1 and 2
        const defaultSpawn = { position: { x: -26.05, y: 0, z: 1.19 } }

        this.focusPoint = {}
        this.focusPoint.trackedPosition = new THREE.Vector3(defaultSpawn.position.x, 0, defaultSpawn.position.z)
        ...
        this.game.inputs.events.on('actionStart', (action) =>
        {
            ...
            {
                // In game mode, don't snap camera back to car on input
                const store = this.game.uiOverlay?.store
                if(store && store.gameMode) return
                this.focusPoint.isTracking = true
            }
        })
    ...
    setSpherical()
    {
        ...
        this.spherical.theta = -1.28

        this.spherical.radius = {}
        this.spherical.radius.edges = { min: 30, max: 60 } // Increased by 2x
