...
export class Options
{
    ...
    setQuality()
    {
        ...
        // Auto Quality toggle
        const autoQualityElement = this.element.querySelector('.js-auto-quality')
        const autoQualityText = autoQualityElement.querySelector('span')
        
        autoQualityElement.addEventListener('click', () =>
        {
            this.game.quality.auto = !this.game.quality.auto
            autoQualityText.textContent = this.game.quality.auto ? 'On' : 'Off'
            
            if (this.game.quality.auto)
            {
                this.game.quality.startMonitoring()
            }
            else
            {
                this.game.quality.stopMonitoring()
            }
        })

        // Sun Intensity slider
        const sunIntensityElement = this.element.querySelector('.js-sun-intensity')
        sunIntensityElement.value = this.game.lighting.light.intensity
        sunIntensityElement.addEventListener('input', (e) =>
        {
            const value = parseFloat(e.target.value)
            this.game.lighting.light.intensity = value
        })

        // Bloom Strength slider
        const bloomElement = this.element.querySelector('.js-bloom-strength')
        bloomElement.value = this.game.rendering.bloomPass?.strength.value || 0.25
        bloomElement.addEventListener('input', (e) =>
        {
            const value = parseFloat(e.target.value)
            this.game.rendering.setBloom({ strength: value })
        })
    }
