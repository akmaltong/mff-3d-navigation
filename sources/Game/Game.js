...
import { FocusPointGizmo } from './FocusPointGizmo.js'
import { NavMeshSystem } from './NavMeshSystem.js'
import { UIOverlay } from './UI/UIOverlay.js'
import { zones } from './UI/data/mockData.js'
...
export class Game
{
    ...
    async init()
    {
        ...
        this.dayCycles.properties = {
            ...
            fogNearRatio: { value: 0.5 },
            fogFarRatio: { value: 1.5 },
        }
        ...
        this.view = new View()
        this.focusPointGizmo = new FocusPointGizmo()
        ...
        // Load saved zone positions BEFORE creating UI (so markers spawn at correct positions)
        this.settingsStorage.loadZonePositions(zones)

        // Create POI manager
        this.poiManager = new POIManager()

        // Load NavMesh for pathfinding
        this.navMeshSystem = new NavMeshSystem()
        this.navMeshSystem.load('Navmesh.glb').catch(e => console.warn('NavMesh load failed:', e))

        // Create MFF UI overlay
        this.uiOverlay = new UIOverlay()

        // Apply saved settings immediately (not delayed)
        this.settingsStorage.applySettings()

        // Default: game mode ON, focus gizmo hidden
        this.focusPointGizmo.setVisible(false)
        this.uiOverlay.store.set('gameMode', true)

        // Free camera — mouse drag to move
        this.view.focusPoint.isTracking = false
        this.view.focusPoint.magnet.active = false

        // Move car to camera focus point (target) so it starts where the camera looks
        const fpX = this.view.focusPoint.position.x
        const fpZ = this.view.focusPoint.position.z
        this.physicalVehicle.moveTo({ x: fpX, y: 1, z: fpZ }, 0)
        this.player.basePosition.set(fpX, 1, fpZ)
    }
