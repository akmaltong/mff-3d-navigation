...
export class Lighting
{
    constructor()
    {
        ...
        this.useDayCycles = false // Disable day cycles for manual light control
        this.phi = 0.63
        this.theta = 0.72
        ...
        // Light colors
        this.sunColor = new THREE.Color('#ffffff')
        ...
        {
            this.setQuality(this.game.quality.level)
        })
    ...
    setLight()
    {
        ...
        this.light.color = this.sunColor

        this.game.scene.add(this.light)
        this.game.scene.add(this.light.target)

        // Add ambient light for better visibility - increased intensity
        this.ambientLight = new THREE.AmbientLight(0xffffff, 2.0)
        this.ambientLight.color = this.ambientColor
    ...
    setDebugPanel()
    {
        if(this.game.debug.active)
        {
            const lightingFolder = this.game.debug.panel.addFolder({
                title: 'Lighting',
                expanded: true,
            })

            // Sun light intensity slider
            lightingFolder.addBinding(this.light, 'intensity', {
                label: 'Sun Intensity',
                min: 0,
                max: 20,
                step: 0.1,
            ...
            lightingFolder.addBinding(this.ambientLight, 'intensity', {
            ...
            lightingFolder.addBinding(this.spherical, 'phi', {
                label: 'Sun Phi (height)',
                min: 0,
                max: Math.PI * 0.5,
                step: 0.01,
            })

            lightingFolder.addBinding(this.spherical, 'theta', {
                label: 'Sun Theta (rotation)',
                min: 0,
                max: Math.PI * 2,
                step: 0.01,
            })
            ...
            // Sun color
            lightingFolder.addBinding(this.sunColor, 'r', { label: 'Sun Color R', min: 0, max: 1, step: 0.01 })
            lightingFolder.addBinding(this.sunColor, 'g', { label: 'Sun Color G', min: 0, max: 1, step: 0.01 })
            lightingFolder.addBinding(this.sunColor, 'b', { label: 'Sun Color B', min: 0, max: 1, step: 0.01 })

            // Ambient color
            lightingFolder.addBinding(this.ambientColor, 'r', { label: 'Ambient Color R', min: 0, max: 1, step: 0.01 })
            lightingFolder.addBinding(this.ambientColor, 'g', { label: 'Ambient Color G', min: 0, max: 1, step: 0.01 })
            lightingFolder.addBinding(this.ambientColor, 'b', { label: 'Ambient Color B', min: 0, max: 1, step: 0.01 })
        }
    }
     ...
     * @param {string|THREE.Color} colorValue
     * @param {number} intensity
     */
    setSunColor(colorValue, intensity = null)
    {
        this.sunColor.set(colorValue)
        this.light.color = this.sunColor
        if (intensity !== null)
        {
    ...
    /**
     * Установить цвет ambient света
     * @param {string|THREE.Color} colorValue
     * @param {number} intensity
     */
    setAmbientColor(colorValue, intensity = null)
    {
        this.ambientColor.set(colorValue)
        this.ambientLight.color = this.ambientColor
        if (intensity !== null)
        {
    ...
    update()
    {
        // Spherical coordinates - only update if useDayCycles is true
