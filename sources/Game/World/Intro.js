...
export class Intro
{
    ...
    destroy()
    {
        ...
        // Geometries
        if(this.circle?.mesh?.geometry)
            this.circle.mesh.geometry.dispose()

        if(this.soundButton?.mesh?.geometry)
            this.soundButton.mesh.geometry.dispose()

        if(this.text?.mesh?.geometry)
            this.text.mesh.geometry.dispose()

        // Materials
        if(this.circle?.mesh?.material)
            this.circle.mesh.material.dispose()

        if(this.soundButton?.mesh?.material)
            this.soundButton.mesh.material.dispose()

        if(this.text?.mesh?.material)
            this.text.mesh.material.dispose()

        // Textures
        if(this.game.resources?.soundTexture)
