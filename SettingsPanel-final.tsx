...
type LightingModeTab = 'sky' | 'baked' | 'custom'
...
export default function SettingsPanel() {
  ...
  const [lightingModeTab, setLightingModeTab] = useState<LightingModeTab>(store.lightingMode as LightingModeTab)
  ...
  const handleLightingModeChange = (mode: LightingModeTab) => {
    setLightingModeTab(mode)
    store.setLightingMode(mode)
  }
        ...
        {activeTab === 'lighting' && (
          <div className="flex flex-col gap-4">
            ...
            <div style={{ marginBottom: '16px' }}>
              <div style={{ fontSize: '10px', fontWeight: 600, color: 'rgba(255,255,255,0.5)', letterSpacing: '0.5px', textTransform: 'uppercase' as const, marginBottom: '8px' }}>
                Режим освещения
              </div>
              <div className="flex gap-2">
                <button
                  onClick={() => handleLightingModeChange('sky')}
                  style={{
                    flex: 1,
                    padding: '10px 14px',
                    borderRadius: '10px',
                    fontSize: '11px',
                    fontWeight: 600,
                    color: lightingModeTab === 'sky' ? '#fff' : 'rgba(255,255,255,0.6)',
                    background: lightingModeTab === 'sky' ? 'linear-gradient(145deg, rgba(59,130,246,0.4) 0%, rgba(59,130,246,0.2) 100%)' : 'rgba(255,255,255,0.05)',
                    border: `1px solid ${lightingModeTab === 'sky' ? 'rgba(59,130,246,0.6)' : 'rgba(255,255,255,0.1)'}`,
                    cursor: 'pointer',
                    transition: 'all 0.2s',
                  }}
                  onMouseEnter={(e) => {
                    if (lightingModeTab !== 'sky') e.currentTarget.style.background = 'rgba(255,255,255,0.1)'
                  }}
                  onMouseLeave={(e) => {
                    if (lightingModeTab !== 'sky') e.currentTarget.style.background = 'rgba(255,255,255,0.05)'
                  }}
                >
                  ☀️ Sky (HDRI)
                </button>
                <button
                  onClick={() => handleLightingModeChange('baked')}
                  style={{
                    flex: 1,
                    padding: '10px 14px',
                    borderRadius: '10px',
                    fontSize: '11px',
                    fontWeight: 600,
                    color: lightingModeTab === 'baked' ? '#fff' : 'rgba(255,255,255,0.6)',
                    background: lightingModeTab === 'baked' ? 'linear-gradient(145deg, rgba(139,92,246,0.4) 0%, rgba(139,92,246,0.2) 100%)' : 'rgba(255,255,255,0.05)',
                    border: `1px solid ${lightingModeTab === 'baked' ? 'rgba(139,92,246,0.6)' : 'rgba(255,255,255,0.1)'}`,
                    cursor: 'pointer',
                    transition: 'all 0.2s',
                  }}
                  onMouseEnter={(e) => {
                    if (lightingModeTab !== 'baked') e.currentTarget.style.background = 'rgba(255,255,255,0.1)'
                  }}
                  onMouseLeave={(e) => {
                    if (lightingModeTab !== 'baked') e.currentTarget.style.background = 'rgba(255,255,255,0.05)'
                  }}
                >
                  🏠 Baked (Lightmap)
                </button>
                <button
                  onClick={() => handleLightingModeChange('custom')}
                  style={{
                    flex: 1,
                    padding: '10px 14px',
                    borderRadius: '10px',
                    fontSize: '11px',
                    fontWeight: 600,
                    color: lightingModeTab === 'custom' ? '#fff' : 'rgba(255,255,255,0.6)',
                    background: lightingModeTab === 'custom' ? 'linear-gradient(145deg, rgba(234,179,8,0.4) 0%, rgba(234,179,8,0.2) 100%)' : 'rgba(255,255,255,0.05)',
                    border: `1px solid ${lightingModeTab === 'custom' ? 'rgba(234,179,8,0.6)' : 'rgba(255,255,255,0.1)'}`,
                    cursor: 'pointer',
                    transition: 'all 0.2s',
                  }}
                  onMouseEnter={(e) => {
                    if (lightingModeTab !== 'custom') e.currentTarget.style.background = 'rgba(255,255,255,0.1)'
                  }}
                  onMouseLeave={(e) => {
                    if (lightingModeTab !== 'custom') e.currentTarget.style.background = 'rgba(255,255,255,0.05)'
                  }}
                >
                  🎨 Custom (Folio)
                </button>
              </div>
              <div style={{ fontSize: '9px', color: 'rgba(255,255,255,0.4)', marginTop: '8px', lineHeight: 1.4 }}>
                {lightingModeTab === 'sky' && '☀️ HDRI карта окружения для реалистичного освещения'}
                {lightingModeTab === 'baked' && '🏠 Запечённое освещение из lightmap + ambient свет'}
                {lightingModeTab === 'custom' && '🎨 DirectionalLight + AmbientLight с полной настройкой (как в Bruno Simon Folio)'}
              </div>
            </div>

            {/* Custom Lighting Controls - показываем только в custom режиме */}
            {lightingModeTab === 'custom' && (
              <div className="flex items-start gap-6">
                {/* Left column - Sun and Ambient */}
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '10px', fontWeight: 600, color: 'rgba(255,200,120,0.7)', letterSpacing: '0.5px', textTransform: 'uppercase' as const, marginBottom: '10px' }}>
                    🎨 Custom Lighting (Folio)
                  </div>
                  <SettingsSlider 
            ...
            {lightingModeTab !== 'custom' && (
              <div style={{ 
                ...
                <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.5)', marginBottom: '8px' }}>
                  {lightingModeTab === 'sky' 
                    ? '🌤️ В режиме Sky используются HDRI карты для реалистичного освещения. Настройте HDRI во вкладке эффектов.'
                    : '🏠 В режиме Baked используется запечённое освещение из lightmap. Настройте материал и lightmapIntensity во вкладке материала.'}
                </p>
                <button
                  onClick={() => handleLightingModeChange('custom')}
                  style={{
                    ...
                    fontWeight: 600,
                    color: '#fff',
                    background: 'linear-gradient(145deg, rgba(234,179,8,0.4) 0%, rgba(234,179,8,0.2) 100%)',
                    border: '1px solid rgba(234,179,8,0.6)',
                    cursor: 'pointer',
                    transition: '0.2s',
                  }}
                  onMouseEnter={(e) => e.currentTarget.style.background = 'linear-gradient(145deg, rgba(234,179,8,0.5) 0%, rgba(234,179,8,0.3) 100%)'}
                  onMouseLeave={(e) => e.currentTarget.style.background = 'linear-gradient(145deg, rgba(234,179,8,0.4) 0%, rgba(234,179,8,0.2) 100%)'}
                >
                  🎨 Переключиться на Custom Lighting
                </button>
              </div>
            )}
          </div>
...
function SettingsToggle({ label, value, onChange }: {
  label: string | React.ReactNode; value: boolean; onChange: (v: boolean) => void
}) {
  return (
